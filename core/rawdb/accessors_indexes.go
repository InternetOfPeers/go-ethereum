...
func writeTxLookupEntry(db ethdb.KeyValueWriter, numberBytes []byte, hash common.Hash) {
	log.Info("=== writeTxLookupEntry");
...
func WriteTxLookupEntries(db ethdb.KeyValueWriter, number uint64, hashes []common.Hash) {
	log.Info("=== WriteTxLookupEntries", "number", number, "size", len(hashes));
	numberBytes := new(big.Int).SetUint64(number).Bytes()
	for _, hash := range hashes {
		writeTxLookupEntry(db, numberBytes, hash);
	}
}
...
func WriteTxLookupEntriesByBlock(db ethdb.KeyValueWriter, block *types.Block) {
	log.Info("=== WriteTxLookupEntriesByBlock");
	numberBytes := block.Number().Bytes()
	for _, tx := range block.Transactions() {
		writeTxLookupEntry(db, numberBytes, tx.Hash());
	}
}
